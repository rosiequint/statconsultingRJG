...
We may want to predict the probability of DV based on a certain number of co-wives or beating justification score. To do so, we can set our other control variables (education, wealth, residence) to “typical” values (most common levels). 
...
Here, we have created a hypothetical profile of a woman with the most common education level (no education), wealth index (poorest), residence (urban), and contraception decision (yes), and a typical beating-justification score (median of 2). We will use this profile to predict the probability of DV across different co-wives groups, while holding these other characteristics constant.

### Step 7b — Predicted Pr(DV) by co-wives group (holding others constant)
...
### Step 7c — Predicted Pr(DV) across beating-justification scores, by co-wives group
...
ggplot(new_score, aes(x = beating_justified_score, y = pred_p,
  ...
  labs(
    ...
    y = "Predicted probability of any DV",
    title = "Predicted Pr(any DV) across beating-justification scores",
    subtitle = paste0(),
...
This plot shows how predicted DV probability changes as the DV-justification score increases from 0 to 5, separately for each co-wives group, again holding other covariates constant at the typical values described above. All lines trend upward, meaning higher justification scores correspond to higher predicted DV probability. The lines are fairly similar across co-wives groups, suggesting the relationship between score and DV is broadly consistent across these groups, which aligns with the odds ratio of about 1.05 from the model.  
