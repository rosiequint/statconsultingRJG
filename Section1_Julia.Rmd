...
# create a plain data frame from the final survey object
table1_df <- dhs_surv$variables %>%
  mutate(
    ...
    cowives_cat = factor(cowives_cat, levels = c("0", "1", "2+")),
    contraception_dec = factor(v822, levels = c(0, 1), labels = c("No", "Yes")),
    education = factor(v106, levels = c(0, 1, 2, 3),
                       labels = c("None", "Primary", "Secondary", "Higher")),
    wealth = factor(v190, levels = c(1, 2, 3, 4, 5),
                    labels = c("Poorest", "Poorer", "Middle", "Richer", "Richest")),
    residence = factor(v025, levels = c(1, 2),
...
# rebuild a survey design object for the final analytic sample
table1_design <- svydesign(
  ids = ~caseid,
  strata = ~v001,
  weights = ~v005,
  data = table1_df
...
tbl1 <- tbl_svysummary(
  ...
  add_p(
    test = list(
      beating_justified_score ~ "svy.wilcox.test",
