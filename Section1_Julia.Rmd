...
```{r}
svytable(~d105d, design = dhs_surv) %>%
  prop.table() %>%
  { . * 100 } %>%
  round(digits = 2)
```
...
svytable(~beating_justified_score, design = dhs_surv) %>%
  prop.table() %>%
  { . * 100 } %>%
  round(digits = 2)
```
...
svytable(~v812, design = dhs_surv) %>%
  prop.table() %>%
  { . * 100 } %>%
  round(digits = 2)

svytable(~v813, design = dhs_surv) %>%
  prop.table() %>%
  { . * 100 } %>%
  round(digits = 2)
```
